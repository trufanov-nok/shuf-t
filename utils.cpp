...
void initCommandOptionsParser(QCommandLineParser &parser)
{
    ...
    parser.setApplicationDescription("This application shuffles the input file lines skipping (optionaly) the header. It's optimized for files bigger than available RAM. Shuffling performed in 3 steps:\n1. The file is scanned and offsets of all lines are stored in RAM.\n2. Offsets are shuffled (Fisher–Yates algorythm).\n3. Iteratively shuf-t takes the offsets of first N shuffled lines and sort their offsets ascendingly. Lines are read from input file (only seeking forward is used as offsets are sorted) and written to memory allocated buffer in shuffled order. Then buffer is written to output file. Buffer size can be adjusted by user based on available amount of free RAM. Value N is choosen based on lines length and buffer size at the beginning of each iteration.");
    ...
    parser.addOption(QCommandLineOption(QStringList() << "b" << "buffer", "Buffer size in MiB for read operations. Default is 100 MiB.", "SIZE", "100"));
