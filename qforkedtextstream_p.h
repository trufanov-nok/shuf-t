#ifndef QFORKEDTEXTSTREAM_P_H
#define QFORKEDTEXTSTREAM_P_H
/****************************************************************************
**
...
class QForkedDeviceClosedNotifier : public QObject
{
    Q_OBJECT
public:
    inline QForkedDeviceClosedNotifier()
    { }

    inline void setupDevice(QForkedTextStream *stream, QIODevice *device)
    {
        disconnect();
        if (device)
...
private:
    QForkedTextStream *stream;
};
#endif

class QForkedTextStreamPrivate
{
    Q_DECLARE_PUBLIC(QForkedTextStream)
public:
    ...
    public:
        ...
        int fieldWidth;
        QChar padChar;
        QForkedTextStream::FieldAlignment fieldAlignment;
        QForkedTextStream::RealNumberNotation realNumberNotation;
        QForkedTextStream::NumberFlags numberFlags;
    };

    QForkedTextStreamPrivate(QForkedTextStream *q_ptr);
    ~QForkedTextStreamPrivate();
    void reset();

    // device
    QIODevice *device;
#ifndef QT_NO_QOBJECT
    QForkedDeviceClosedNotifier deviceClosedNotifier;
#endif
    ...
    // status
    QForkedTextStream::Status status;
    QLocale locale;
    QForkedTextStream *q_ptr;
